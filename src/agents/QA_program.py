...
from src.state import State, QA_response_pro
...
#     def retriever_def(self, query , threshould: float = 0.5):
...
#         filtered_docs = [doc for doc, score in docs if score >= threshould]
#         return filtered_docs

#     def rerank_documents(self, query , docs , top_k = 4):
#         if not docs:
#             print("not documents")
#             return []
...
#         # print("\nPrompt QA_Program: ", prompt)    

#         # humnan_messsage = [mes for mes in state["messages"] if mes.type =="human"]
#         # query = humnan_messsage[-1].content

#         # query = "Chương mệnh đề tập hợp là học những gì, hãy giới thiệu về các khái niệm hay định của nó , nó có liên quan gì đến các chương sau của toán 10 không, các dạng bài tập của mệnh đề tập hợp gồm những dạng nào?"
#         # query = "tích phần là gì có các dạng nào của tích phân, giải tự luận bap gồm các bước nào?"
#         # query = "MỘT SỐ YẾU TỐ THỐNG KÊ VÀ XÁC SUẤT, MỆNH ĐỀ TẬP HỢP"
        #   query ="Mệnh đề là gì có các dạng bài tập nào , tính chất của mệnh đề"

#         # B1: lấy context qua retriever
...
#         # B2: re-rank
#         # reranked_docs = self.rerank_documents(query, docs, top_k=4)

#         # print("\n=== Top reranked docs chọn để build prompt ===")
