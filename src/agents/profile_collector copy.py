...
class ProfileCollector:
    ...
    def __call__(self, state: State) -> dict:
        ...
        try:
            ...
            # def is_profile_completed(profile: AgentProfile) -> bool:
            #     # lấy tất cả giá trị field từ pydantic model
            #     filled_fields = [v for v in profile.dict().values() if v not in (None, "", [])]
            #     return len(filled_fields) >= 10


            def generate(state: State):
                print("\nProfileCollector generate is running...")
                ...
                return {
                    ...
                    # "messages":"hoàn thành việc thu thập hồ sơ học tập.",
            ...
            #     if not last_tool_msg.artifact:
            #         return {"messages": ["artifact none"]}

            #     profile_data = last_tool_msg.artifact
            #     profile = AgentProfile(**profile_data)

            #     profile_done = is_profile_completed(profile)

            #     return {
            #         "profile_user": profile.dict(),
            #         "profile_completed": profile_done,
            #         "messages": [
            #             AIMessage(
            #                 content="✅ Đã thu thập đủ hồ sơ." if profile_done else "⚠️ Hồ sơ chưa đủ, cần bổ sung."
            #             )
            #         ],
            #     }
